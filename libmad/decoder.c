...
//  decoder->sync         = 0; // why is this set to 0?
...
enum mad_flow receive_io_blocking(int fd, void *buffer, size_t len)
{
  ...
  printf ("decoder: receive_io_blocking()\n");
...
enum mad_flow receive(int fd, void **message, unsigned int *size)
{
  ...
  if (result == MAD_FLOW_CONTINUE) {
    ...
    if (*size > 0) {
      if (*message == 0) {
	*message = malloc(*size);
printf ("decoder: malloc big success\n");
	if (*message == 0)
	  return MAD_FLOW_BREAK;
      }
...
int run_sync(struct mad_decoder *decoder)
{
  ...
  printf ("begin mad_stream_init (%p)\n", stream);
  mad_stream_init(stream);
  printf ("begin mad_frame_init (%p)\n", frame);
  mad_frame_init(frame);
  printf ("begin mad_synth_init (%p)\n", synth);
...
printf ("decoder->sync: malloc big success %p\n", decoder->sync);
  if (decoder->sync == 0)
    return -1;
